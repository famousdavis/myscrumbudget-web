    ...
    sections: [
      {
        title: 'Added',
        items: [
          'Shared projects can now show everyone the same costs — but only after you save each one. When the OWNER of a project saves it, the app records the labor rates, holidays and discount rate that project is being budgeted with onto the project itself. Anyone you have shared it with is then priced with YOUR rates instead of their own, so the Estimate at Completion you see and the one they see finally agree. Before this, six collaborators could open one project and see six different totals.',
          'Owners must open and save each project once for this to take effect. Nothing is applied retroactively — a project you never open again keeps behaving as it did. Any ordinary edit counts as a save, so most projects will pick this up simply through normal use.',
    ...
    sections: [
      {
        ...
        items: [
          ...
          'One trade worth stating plainly, because it is a cost and not a detail. Setting an unreadable entry aside and carrying it through means it is preserved as a value, not as the exact original text: the first save after the damage rewrites the whole entry, and a handful of things do not survive a rewrite unchanged — a number too large to represent becomes empty, negative zero becomes zero, whole numbers beyond about sixteen digits lose their last digits, and repeated keys collapse to one. Eight of ten shapes we measured changed in some way, four of them meaningfully. Nothing the app reads is affected, because these entries are by definition ones it cannot read — but if you later recover one by hand you get the value as the browser parsed it, not the bytes originally stored. The alternative was to refuse to save anything at all until the damage was resolved, which preserves the original text exactly and leaves you unable to use the app; this is the trade we chose, and it is why.',
        ],
      },
    ],
  },
    ...
    sections: [
      {
        title: 'Fixed',
        items: [
          "A release check had been quietly skipping itself, in exactly the situation releases are normally prepared in. The gate confirms that this project's internal notes file declares the same version number as everything else, so those notes cannot drift out of date unnoticed.",
          'Why it was skipping: the check skips when the notes file is not present, which is correct on the automated build server, where the file is deliberately excluded and genuinely absent. But the file is also missing for a second, unrelated reason — when a release is prepared in a separate working copy, the tooling that creates that copy leaves excluded files behind. The check saw the same absence, assumed the build-server reason, and skipped, printing a skip line while the gate went green.',
          'What happens now: the check tells the two reasons apart. On the build server it skips as before; otherwise it reads the notes file from the main working copy, so the check runs even when the release is prepared elsewhere. If it still cannot be found, that is a failure rather than a skip.',
        ],
      },
      {
        title: 'Notes',
    ...
    sections: [
      {
        title: 'Changed',
        items: [
          'Tests only. Nothing in the app itself changed — it behaves identically to v0.36.7.',
          ...
          'Worth noting why those tests take the shape they do: a test confirming a bad value is rejected still passes if a good value has gone missing from the list. Only a test confirming each good value is accepted can catch that, so there is now one per value.',
          ...
          'Each new test was verified by deliberately breaking the corresponding check and confirming the right test — and only the right test — failed. No production code changed: the file containing both validators is byte-for-byte identical to v0.36.7.',
        ],
      },
    ],
  },
