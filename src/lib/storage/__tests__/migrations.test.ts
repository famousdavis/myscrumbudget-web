...
describe('Migrations', () => {
  ...
  it('returns data unchanged when no migrations are pending', () => {
    // Seeded from the DATA_VERSION constant (not a literal) so this stays a
    // genuine no-op across future version bumps.
    const data = makeAppState({ version: DATA_VERSION });
    const result = runMigrations(data, DATA_VERSION);
    expect(result).toEqual(data);
  });

  it('exports current version constant', () => {
    expect(DATA_VERSION).toBe('0.16.0');
  });
  ...
  it('migrates imported older data through runMigrations', () => {
    ...
    expect(migrated.version).toBe('0.16.0');
  ...
  it('migrates v0.3.0 to v0.4.0 (creates Baseline for project without reforecasts)', () => {
    ...
    // v0.14.0 migration converts YYYY-MM → YYYY-MM-DD using the day component
    // from project.startDate (2026-06-15 → day 15).
    expect(proj.reforecasts[0].startDate).toBe('2026-06-15');
  ...
  describe('v0.11.0 to v0.12.0 (PoolMember.archived optional field)', () => {
    ...
    it('migrates v0.11.0 → v0.12.0 as a no-op for teamPool/projects (post-0.13.0 settings gain violetPercent)', () => {
      ...
      // v0.14.0 migration transforms each reforecast's startDate to YYYY-MM-DD
      // and backfills endDate from project.endDate.
      const expectedProjects = (v11Data as { projects: { reforecasts: { startDate: string }[] }[] }).projects.map((p) => ({
        ...p,
        reforecasts: p.reforecasts.map((rf) => ({
          ...rf,
          startDate: '2026-01-01',
          endDate: '2026-12-31',
        })),
  ...
  describe('v0.15.0 to v0.16.0 (adds optional Project.archived — structural no-op)', () => {
    function makeV15(): unknown {
      return {
        version: '0.15.0',
        settings: {
          discountRateAnnual: 0.03,
    ...
    it('stamps the version and does NOT backfill archived (absent = active)', () => {
      const data = makeV15();
      const result = runMigrations(data as never, '0.15.0');
      expect(result.version).toBe('0.16.0');
      expect(result.projects[0]).not.toHaveProperty('archived');
      // Projects are otherwise untouched (only the top-level version changed).
      expect(result.projects).toEqual((data as { projects: unknown[] }).projects);
    });

    it('preserves an existing archived flag when present', () => {
      const data = makeV15() as { projects: { archived?: boolean }[] };
      data.projects[0].archived = true;
      const result = runMigrations(data as never, '0.15.0');
      expect((result.projects[0] as { archived?: boolean }).archived).toBe(true);
    });

    it('is idempotent when already at 0.16.0', () => {
      const data = { ...(makeV15() as object), version: '0.16.0' } as AppState;
      const result = runMigrations(data, '0.16.0');
      expect(result).toEqual(data);
    });
  });
});
