...
describe('createNewReforecast', () => {
  it('creates an empty reforecast when no source is provided', () => {
    const rf = createNewReforecast('Q3 Reforecast', { startDate: PROJECT_START, endDate: PROJECT_END });
  ...
  it('copies baselineBudget from source reforecast', () => {
    ...
    const rf = createNewReforecast('Reforecast 2', { startDate: PROJECT_START, endDate: PROJECT_END }, source);
  ...
  it('generates unique IDs across calls', () => {
    const rf1 = createNewReforecast('A', { startDate: PROJECT_START, endDate: PROJECT_END });
    const rf2 = createNewReforecast('B', { startDate: PROJECT_START, endDate: PROJECT_END });
    expect(rf1.id).not.toBe(rf2.id);
  });

  it('defaults actualCost to 0 when source is undefined', () => {
    const rf = createNewReforecast('Fresh', { startDate: PROJECT_START, endDate: PROJECT_END }, undefined);
    expect(rf.actualCost).toBe(0);
  });
  ...
  it('does not set actualsThroughDate for fresh reforecast', () => {
    const rf = createNewReforecast('Fresh', { startDate: PROJECT_START, endDate: PROJECT_END });
    expect(rf.actualsThroughDate).toBeUndefined();
  });
  ...
  it('materializes source bucket on copy when source has actualsThroughDate but no historicalCosts', () => {
    ...
    const rf = createNewReforecast('April', { startDate: PROJECT_START, endDate: PROJECT_END }, source);
  ...
  it('chained-copy scenario: bucket value flows correctly through Mar 7 → Mar 14 → Mar 21', () => {
    ...
    const mar14 = createNewReforecast('Mar 14', { startDate: PROJECT_START, endDate: PROJECT_END }, mar7);
